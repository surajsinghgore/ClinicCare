...
import { getSpecialtiesByTreatmentApi, getTreatmentNamesByCityApi, getUniqueCitiesApi, getDoctorsByCityTreatmentAndSpecialtyApi } from '../../Utils/services/apis/CommonApi';
...
const BookAppointment = () => {
    ...
    const handleSpecialtyChange = async (event) => {
        ...
        if (specialty) {
            dispatch(showLoader());
            try {
                const res = await getDoctorsByCityTreatmentAndSpecialtyApi(selectedCity, selectedTreatment, specialty);
                if (res?.success) {
                    setData(res.data);
                }
            } catch (error) {
                            ...
                            <div key={index} className="doctor-card flex items-start py-8 bg-white p-4 shadow rounded-lg hover:shadow-lg transition-shadow">
                                ...
                                <div className="ml-4 flex-grow">
                                    ...
                                    <p className="text-black-600">
                                        <span className='text-[#0148B1] font-semibold'>Treatment name:</span> {doctor.treatmentName || "Not Specified"}
                                    </p>
                                    ...
                                    <Link to={`/doctor-details/${doctor.doctorId}`} className="mt-4 px-4 py-2 text-white bg-[#0148B1] rounded-md shadow">Book Appointment</Link>
                                </div>
                                <div className="ml-auto text-right">
                                    <p className="text-black-600">
                                        <span className='text-[#0148B1] font-semibold'>Experience:</span> {doctor.experience || "Not Available"} years
                                    </p>
                                    <p className="text-black-600">
                                        <span className='text-[#0148B1] font-semibold'>Expertise:</span> {doctor.specialization || "Not Specified"}
                                    </p>
                                </div>
                            </div>
                        ))
